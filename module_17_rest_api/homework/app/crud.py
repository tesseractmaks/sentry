...
def get_obj_by_id(TABLE_NAME, obj_names_column, obj_id: int):
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor: sqlite3.Cursor = conn.cursor()
        # cursor.execute(
        #     f"SELECT * FROM `{TABLE_NAME}` WHERE book_id = '%s'" % book_id,
        # )
        names_column = None
...
def add_obj(TABLE_NAME, obj_class):
    with sqlite3.connect(DATABASE_NAME) as conn:
        ...
        # cursor.execute(
        #     f"""
        #     INSERT INTO `{TABLE_NAME}` (book_name, author) VALUES (?, ?)
        #     """,
...
def update_obj_by_id(TABLE_NAME, obj_class, obj_id):
    ...
    with sqlite3.connect(DATABASE_NAME) as conn:
        ...
        #     SET book_name = ?, author = ? WHERE book_id = ?
        #     """,
        #     (book.book_name, book.author, book_id)
        # )
        conn.commit()
...
#         cursor.execute(
#             f"""
#             SELECT * FROM `{TABLE_NAME}` WHERE [first_name = ?, middle_name = ?, last_name = ?]
#             """,
