...
name_start_letters = "абвгдежзиклмнопрстуфхцчшщэюя".upper()


def add_person_to_school(db_name: str, number_of_person: int):
    with sqlite3.connect(db_name) as conn:
        ...
        for _ in range(number_of_person):
            ...
            name_letter = random.choice(name_start_letters)

            full_name = f"{family_name} {name_letter}."

            class_number = random.randint(1, 11)

            amount_of_money = random.randint(25, 500) * 10
...
def cleanup():
    ...
    hw_dir_name = os.path.dirname(this_file_path)
    for file_name in glob.glob(f"{hw_dir_name}/*.db"):
        print(f"\t{file_name}")
        os.unlink(file_name)


def generate_db(db_name: str, sql_request: str):
